...
macro_rules! gen_stream_data_tests {
    (
        ...
        fn read_list_matches_input() {
            ...
            $test.$add_read($read_type::StreamData(StreamData::List(Some(Value::test_bool(true)))));
            ...
            interface.read_list().expect_err("didn't err on end of input");
        }

        #[test]
        fn read_external_matches_input() {
            let $test = TestCase::new();
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![67])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![68])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalExitCode(Some(Value::test_int(1)))));
            ...
            match interface.read_external_stdout().expect("while reading stdout") {
                Some(buffer) => assert_eq!(buffer, vec![67]),
                None => panic!("unexpected end of stdout stream"),
            }

            match interface.read_external_stderr().expect("while reading stderr") {
                Some(buffer) => assert_eq!(buffer, vec![68]),
                None => panic!("unexpected end of stderr stream"),
            }

            match interface.read_external_exit_code().expect("while reading exit code") {
                Some(value) => assert_eq!(value, Value::test_int(1)),
                None => panic!("unexpected end of exit code stream"),
            }

            match interface.read_external_exit_code().expect("while reading exit code") {
                Some(value) => panic!("unexpected value in exit code stream, expected end: {value:?}"),
                None => (),
            }

            interface.read_external_exit_code().expect_err("no error at end of input");
        }

        #[test]
        fn read_external_streams_out_of_input_order() {
            let $test = TestCase::new();
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![43])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalExitCode(Some(Value::test_int(42)))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![70])))));
        ...
        fn read_external_streams_skip_dropped_stdout() {
            let $test = TestCase::new();
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![1])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![2])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![3])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![42])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![4])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![43])))));
            ...
            interface.read_external_stdout().expect_err("reading from dropped stream should be err");
            assert_eq!(interface.read_external_stderr().unwrap(), Some(vec![42]));
            assert_eq!(interface.read_external_stderr().unwrap(), Some(vec![43]));
            assert!(interface.read.lock().unwrap().1.external_stdout.is_dropped());
            interface.read_external_stdout().expect_err("reading from dropped stream should be err");
        }
        ...
        fn read_external_streams_skip_dropped_stderr() {
            let $test = TestCase::new();
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![1])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![2])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![3])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![42])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStderr(Some(Ok(vec![4])))));
            $test.$add_read($read_type::StreamData(StreamData::ExternalStdout(Some(Ok(vec![43])))));

            let interface = $gen_interface_impl;
            interface.read.lock().unwrap().1 = StreamBuffers::new_external(true, true, true);

            interface.drop_external_stderr();
            interface.read_external_stderr().expect_err("reading from dropped stream should be err");
            assert_eq!(interface.read_external_stdout().unwrap(), Some(vec![42]));
            assert_eq!(interface.read_external_stdout().unwrap(), Some(vec![43]));
            assert!(interface.read.lock().unwrap().1.external_stderr.is_dropped());
            interface.read_external_stderr().expect_err("reading from dropped stream should be err");
        }
        ...
        fn read_external_streams_skip_dropped_exit_code() {
            ...
            interface.read_external_exit_code().expect_err("reading from dropped stream should be err");
            assert_eq!(interface.read_external_stderr().unwrap(), Some(vec![2]));
            assert_eq!(interface.read_external_stderr().unwrap(), Some(vec![3]));
            assert_eq!(interface.read_external_stdout().unwrap(), Some(vec![42]));
            assert!(interface.read.lock().unwrap().1.external_exit_code.is_dropped());
            interface.read_external_exit_code().expect_err("reading from dropped stream should be err");
        }
        ...
        fn read_error_passthrough() {
            ...
            $test.set_read_error(ShellError::IOError { msg: test_msg.into() });

            let interface = $gen_interface_impl;
            interface.read.lock().unwrap().1 = StreamBuffers::new_external(true, true, true);

            match interface.read_external_exit_code().expect_err("succeeded unexpectedly") {
        ...
        fn write_error_passthrough() {
            let $test = TestCase::new();
            let test_msg = "test io error";
            $test.set_write_error(ShellError::IOError { msg: test_msg.into() });

            let interface = $gen_interface_impl;

            match interface.write_list(None).expect_err("succeeded unexpectedly") {
                ShellError::IOError { msg } => assert_eq!(test_msg, msg),
                other => panic!("other error: {other}")
            }
        }
        ...
        fn write_list() {
            let $test = TestCase::new();
            let data = [
                Some(Value::test_int(1)),
                Some(Value::test_int(2)),
                None
            ];
            let interface = $gen_interface_impl;
        ...
        fn write_external_stdout() {
            ...
            let data = [
                Some(Ok(vec![42])),
                Some(Ok(vec![80, 40])),
                Some(Err(ShellError::IOError { msg: "test io error".into() })),
                None
            ];
            let interface = $gen_interface_impl;
            for item in data.iter() {
                interface.write_external_stdout(item.clone()).expect("write failed");
            }
            for item in data.iter() {
                match $test.$get_write() {
                    Some($write_type::StreamData(StreamData::ExternalStdout(read_item))) =>
                        match (item, &read_item) {
                            ...
                            (Some(Err(a)), Some(Err(b))) => assert_eq!(a.to_string(), b.to_string()),
                            (None, None) => (),
        ...
        fn write_external_stderr() {
            ...
            let interface = $gen_interface_impl;
            for item in data.iter() {
                interface.write_external_stderr(item.clone()).expect("write failed");
            }
            for item in data.iter() {
                match $test.$get_write() {
        ...
        fn write_external_exit_code() {
            ...
            let interface = $gen_interface_impl;
            for item in data.iter() {
                interface.write_external_exit_code(item.clone()).expect("write failed");
            }
            for item in data.iter() {
                match $test.$get_write() {
...
fn stream_buffers_default_doesnt_accept_stream_data() {
    ...
    buffers.skip(StreamData::List(Some(Value::test_bool(true))))
        .expect_err("list was accepted");

    buffers.skip(StreamData::ExternalStdout(Some(Ok(vec![]))))
        .expect_err("external stdout was accepted");

    buffers.skip(StreamData::ExternalStderr(Some(Ok(vec![]))))
        .expect_err("external stderr was accepted");

    buffers.skip(StreamData::ExternalExitCode(Some(Value::test_int(1))))
...
fn stream_buffer_not_present_push_err() {
    StreamBuffer::NotPresent.push_back(2).expect_err("should be an error");
}

#[test]
fn stream_buffer_not_present_pop_err() {
    StreamBuffer::<()>::NotPresent.pop_front().expect_err("should be an error");
}
...
fn stream_buffer_dropped_pop_err() {
    StreamBuffer::<()>::Dropped.pop_front().expect_err("should be an error");
}
